...
const Select = ({items}: any) => {
    ...
    return (
        <Listbox as="div" value={selectedItem} onChange={setSelectedItem} className="relative w-full">
            {({ open }) => (
                <React.Fragment>
                    ...
                    <Listbox.Options className="absolute top-[45px] origin-top w-full lg:w-max z-10">
                        {items.map((item: any) => (
                            ...
                            <Listbox.Option key={item.id} value={item} as="li">
                                {({ active, selected }) => (
                                    <p className={`${active ? "bg-blue-600 text-white" : "bg-white text-slate-800"} relative py-3 px-3`}>
                                        {item.name}
                                    </p>
                                )}
                            </Listbox.Option>   
